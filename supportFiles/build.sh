...
if [[ -z "$SUPPORT_DIR" ]]; then
  echo "Error: SUPPORT_DIR is required"
  exit 1
fi
...
case "$SUPPORT_DIR" in
  immortalwrt)
    MOUNT_IMG="immortalwrt.img"
    OUTPUT_NAME="immortalwrt"
    ;;
  istoreos)
    MOUNT_IMG="istoreos.img"
    OUTPUT_NAME="istoreos"
    ;;
  *)
    echo "Error: Unknown SUPPORT_DIR '$SUPPORT_DIR'"
    exit 1
    ;;
esac
...
cp -v /supportFiles/ddd "$HOME/LIVE_BOOT/chroot/usr/bin/ddd-common"
chmod +x "$HOME/LIVE_BOOT/chroot/usr/bin/ddd-common"
cp -v "/supportFiles/$SUPPORT_DIR/ddd" "$HOME/LIVE_BOOT/chroot/usr/bin/ddd"
chmod +x "$HOME/LIVE_BOOT/chroot/usr/bin/ddd"
cp -v /supportFiles/sources.list "$HOME/LIVE_BOOT/chroot/etc/apt/sources.list"

cleanup() {
    umount "$HOME/LIVE_BOOT/chroot/proc" 2>/dev/null || true
    umount "$HOME/LIVE_BOOT/chroot/dev" 2>/dev/null || true
    umount "$HOME/LIVE_BOOT/chroot/sys" 2>/dev/null || true
}
trap cleanup EXIT
...
mv -v "$HOME/LIVE_BOOT/chroot/packages.txt" "/output/${OUTPUT_NAME}-packages.txt"
...
cp "/mnt/$MOUNT_IMG" "$HOME/LIVE_BOOT/chroot/mnt/"
...
VMLINUZ=$(ls "$HOME"/LIVE_BOOT/chroot/boot/vmlinuz-* | sort -V | tail -1)
INITRD=$(ls "$HOME"/LIVE_BOOT/chroot/boot/initrd.img-* | sort -V | tail -1)
cp -v "$VMLINUZ" "$HOME/LIVE_BOOT/staging/live/vmlinuz"
cp -v "$INITRD" "$HOME/LIVE_BOOT/staging/live/initrd"

echo Copy boot config files
cp -v "/supportFiles/$SUPPORT_DIR/isolinux.cfg" "$HOME/LIVE_BOOT/staging/isolinux/isolinux.cfg"
cp -v "/supportFiles/$SUPPORT_DIR/grub.cfg" "$HOME/LIVE_BOOT/staging/boot/grub/grub.cfg"
...
echo Copy output
cp -v "$HOME/LIVE_BOOT/debian-custom.iso" "/output/${OUTPUT_NAME}${VERSION_SUFFIX}.iso"
chmod -v 666 "/output/${OUTPUT_NAME}${VERSION_SUFFIX}.iso"
ls -lah /output
